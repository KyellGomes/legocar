...
void geradeaus() {
  ...
  analogWrite(ENA, 230);     //maximale Geschwindigkeit ist 255!!!
} 
//---------  
void Bremse() {
...
*/    
  ...
  //if(((R>=60) ) )
  if((R>80) && (Lenkrichtung == 1) && (millis() - Kurvenzeit > Geradezeit))
  {
       
   digitalWrite(AMPELrot,HIGH); 
   Lenkung.write(115);        //Wenn Messung größer als 60 dann voll nach links einlenken
   Kurven = Kurven+1;
  ...
  //if((L>=60))
  if((L>80) && (Lenkrichtung == -1) && (millis() - Kurvenzeit > Geradezeit))
  {
   digitalWrite(AMPELgelb,HIGH); 
   Lenkung.write(65);        //Wenn Messung größer als 1000 dann voll nach rechts einlenken
   Kurven = Kurven+1;
   delay (Kurvenfahrzeit); 
   Kurvenzeit = millis();
   vorne();
 ...
 if (Lenkrichtung == 1) 
 {
    a = (R - 25)*0.27 + 90;         //a ist der Rechte Wert minus den Linke Wert Mal Faktor + 95(Mitte der Lenkung vom Servo)
 ...
 {
    a = (25 - L)*0.27 + 90;         //a ist der Rechte Wert minus den Linke Wert Mal Faktor + 95(Mitte der Lenkung vom Servo)
      if(a>125){                      //Abhängig davon lenkt er stärker oder schwächer in den Positiven oder Negativen Bereich
      a = 125;
      }
